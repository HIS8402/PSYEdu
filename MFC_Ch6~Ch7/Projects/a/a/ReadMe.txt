...
a.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CaApp 응용 프로그램
    클래스를 선언합니다.

a.cpp
    응용 프로그램 클래스 CaApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
aDoc.h, aDoc.cpp - 문서
    이 파일에는 CaDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CaDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.

aView.h, aView.cpp - 문서 뷰
    이 파일에는 CaView 클래스가 들어 있습니다.
    CaView 개체는 CaDoc 개체를 보는 데 사용됩니다.

res\aDoc.ico
    CaDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다. 이 아이콘은 주 리소스 파일인 a.rc에
    의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 a.pch와
